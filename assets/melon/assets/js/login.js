	  ...
	  if ($.validator) {		
		  $('.login-form').validate({				
			  ...
			  submitHandler: function (form) {
				 ...
				 var url = window.location.origin+"/login/signin/";
				 //console.log(data);
				 $.ajax({
					 type: "POST",
					 data: data,
					 ...
					 success: function(result){	 
					   if(result){
						 ...
						 $('.login-form .alert-danger').html("Wrong Password. Try Again").show().effect("shake","fast").delay(1000).fadeOut("slow");
						 $('.password').focus().val('').valid();
						 console.log(result); 
					   }
					 }
				 });
